...
if [ $FAIL -eq 0 ]; then
    echo ${DELIMITER}
    echo -e "${GREEN}s21_grep tests SUCCEEDED!${RESET}"
    echo ${DELIMITER}
    echo "s21_grep tests SUCCEEDED" > ../s21_grep_result.txt
    rm -rf ${GREP_OUTPUT} ${S21_GREP_OUTPUT}
else
    echo ${DELIMITER}
    echo -e "${RED}s21_grep tests FAILED!${RESET}"
    echo ${DELIMITER}
    echo "s21_grep tests FAILED" > ../s21_grep_result.txt
    rm -rf ${GREP_OUTPUT} ${S21_GREP_OUTPUT}
fi